...
int adventurer_effect(int player, struct gameState *state)
{
  ...
  // Bug caused by never reaching the zero index of the tempHand
  while (z > 0)
  {
    state->discard[player][state->discardCount[player]++] = tempHand[z]; // discard all cards in play that have been drawn
    z--;
  }
...
int great_hall_effect(int player, int handPos, struct gameState *state)
{
  // +1 Card
  drawCard(player, state);
  ...
  // Bug Caused by returning a negative value
  return -1;
}
...
int sea_hag_effect(int currentPlayer, struct gameState *state)
{
  for (int player = 0; player < state->numPlayers; player++)
  {
    if (player != currentPlayer)
    {
      ...
      // Bug Caused by Index Out of Range
      state->deck[player][state->deckCount[player]] = curse; // Top card now a curse card
    }
  }
...
int smithy_effect(int player, int handPos, struct gameState *state)
{
  // +3 Cards
  for (int i = 0; i <= 3; i++) // Bug Caused by looping 4 times
  {
    drawCard(player, state);
  }
...
int steward_effect(int player, int handPos, int playerChoice, int discardedCard1, int discardedCard2, struct gameState *state)
{
  switch (playerChoice)
  {
  case 1:
  ...
  case 2:
    ...
    state->coins += 2;
  default:
    // Trash 2 Cards from player's hand
    discardCard(discardedCard1, player, state, 1);
    discardCard(discardedCard2, player, state, 1);
    break;
  }
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  case smithy:
    return smithy_effect(currentPlayer, handPos, state);
  ...
  case great_hall:
    return great_hall_effect(currentPlayer, handPos, state);
  ...
  case steward:
    return steward_effect(currentPlayer, handPos, choice1, choice2, choice3, state);
